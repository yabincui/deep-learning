...
def load_data():
    ...
    the validata_data and the test_data.

    The ``training_data`` is returned as a tuple with two entries.
    The first entry contains the actual training images. This is
    a numpy ndarray with 50000 entries. Each entry is, in turn, a
    numpy ndarray with 784 values, representing the 28 * 28 = 784
    pixels in a single MNIST image.

    The second entry in the ``training_data`` tuple is a numpy ndarray
    containing 50000 entries. Those entries are just the digit values
...
def load_data_wrapper():
    ...
    containing the input images. ``y`` is a 10-dimensional
    ...
    10000 2-tuples ``(x, y)``. In each case, ``x`` is a 784-dimensional
