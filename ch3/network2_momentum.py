...
class Network(object):
    def __init__(self, sizes, cost=CrossEntropyCost):
        ...
        self.cost = cost
        self.velocity_biases = [np.zeros((y, 1)) for y in sizes[1:]]
        self.velocity_weights = [np.zeros((y, x)) for x, y in zip(sizes[:-1], sizes[1:])]
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n, momentum_velocity_factor):
        ...
        self.velocity_weights = [momentum_velocity_factor * vw - (eta/len(mini_batch))*nw
                                for vw, nw in zip(self.velocity_weights, nabla_w)]
        self.velocity_biases = [momentum_velocity_factor * vb - (eta/len(mini_batch))*nb
                                for vb, nb in zip(self.velocity_biases, nabla_b)]

        weight_decay = 1 - eta * lmbda / n
        self.weights = [weight_decay * w+vw
                        for w, vw in zip(self.weights, self.velocity_weights)]
